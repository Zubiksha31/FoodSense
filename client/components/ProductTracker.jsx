...
const ProductTracker = () => {
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      formData.append('OCREngine', '2'); // Using OCR Engine 2 for better results
      ...
      if (result.ParsedResults && result.ParsedResults.length > 0) {
        ...
        if (parsedTextResult) {
          ...
          const datePatterns = [
            /(EXP|GXP|EXPIRE|EXPIRY)[\s:]*([0-9]{2}[\/\-][0-9]{2}(?:[\/\-][0-9]{2,4})?)/i, // MM/DD/YYYY or MM-DD-YYYY
            /(EXP|GXP|EXPIRE|EXPIRY)[\s:]*([0-9]{2}[\/\-][0-9]{4})/i, // MM/YYYY
            /([0-9]{2}[\/\-][0-9]{2}[\/\-][0-9]{2,4})/i, // DD/MM/YYYY or MM/DD/YYYY
          ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Convert image to base64 string if needed for storing
      let imageData = null;
      if (imageFile) {
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              {loading && (
                ...
                >
                  <motion.div
                    className="animate-spin rounded-full h-6 w-6 border-2 border-blue-200 border-t-blue-600"
                    animate={{ rotate: 360 }}
              ...
              {error && (
                <motion.div
                  className="bg-red-50/70 backdrop-blur-md text-red-600 p-3 rounded-xl text-sm"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ duration: 0.3 }}
                >
              ...
              {parsedText && (
                <motion.div
                  className="bg-white/30 backdrop-blur-md rounded-xl p-4"
              ...
              <input
                ...
                className="bg-white/30 backdrop-blur-md border border-gray-200 p-3 rounded-xl text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-300"
                required
              />
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                type="submit"
                className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white p-3 rounded-xl shadow-md font-medium transition-all duration-300"
                disabled={loading}
              >
                Add Product
              </motion.button>
          ...
          >
            ...
            {loadingProducts ? (
              <div className="flex justify-center py-8">
                <motion.div
                  className="animate-spin rounded-full h-8 w-8 border-2 border-indigo-200 border-t-indigo-600"
                  animate={{ rotate: 360 }}
                  transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}
                />
              </div>
                ...
                <AnimatePresence>
                  {products.map((product, index) => {
                    ...
                    return (
                      <motion.li
                        key={product._id}
                        className="bg-white/30 backdrop-blur-md p-4 rounded-xl flex justify-between items-center shadow-sm"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -20 }}
                        transition={{
